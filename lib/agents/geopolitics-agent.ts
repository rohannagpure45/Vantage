...
import { GeopoliticsSchema, type GeopoliticsOutput } from "./schemas";
import { GEOPOLITICS_SYSTEM_PROMPT } from "./system-prompts";
import type { OrchestratorOutput } from "./schemas";
...
export async function runGeopoliticsAgent(
  orchestratorOutput: OrchestratorOutput,
  gdeltContext: string,
  options: AgentOptions = {}
): Promise<{ narrative: string; structured: GeopoliticsOutput }> {
  ...
  const textStream = streamText({
    ...
    system: GEOPOLITICS_SYSTEM_PROMPT,
    prompt,
    temperature,
    maxTokens,
    onChunk: ({ chunk }) => {
